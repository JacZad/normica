"""
Narzędzia do analizy i optymalizacji jakości chunkingu.
"""
...
class ChunkingAnalyzer:
    ...
    def analyze_keyword_richness(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "avg_keywords_per_chunk": len(all_keywords) / len(self.documents),
            "chunks_with_keywords": chunks_with_keywords,
            "keyword_coverage": chunks_with_keywords / len(self.documents),
    ...
    def find_potential_improvements(self) -> List[Dict[str, Any]]:
        ...
        unknown_ratio = type_stats.get("type_distribution", {}).get("unknown", 0) / len(self.documents)
    ...
    def visualize_analysis(self) -> None:
        """Tworzy wizualizacje analizy."""
        fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(15, 12))
        
        # 1. Histogram długości chunków
        lengths = self.analysis_results["distribution"]["length_distribution"]
        ax1.hist(lengths, bins=30, alpha=0.7, color='skyblue', edgecolor='black')
        ax1.set_title('Rozkład długości chunków')
        ax1.set_xlabel('Długość (znaki)')
        ax1.set_ylabel('Liczba chunków')
        ax1.axvline(np.mean(lengths), color='red', linestyle='--', label=f'Średnia: {np.mean(lengths):.0f}')
        ax1.legend()
        
        # 2. Wykres kołowy typów chunków
        type_data = self.analysis_results["types"]["type_distribution"]
        ax2.pie(type_data.values(), labels=type_data.keys(), autopct='%1.1f%%', startangle=90)
        ax2.set_title('Rozkład typów chunków')
        
        # 3. Top słowa kluczowe
        keyword_data = dict(self.analysis_results["keywords"]["most_common_keywords"][:10])
        if keyword_data:
            ax3.barh(list(keyword_data.keys()), list(keyword_data.values()))
            ax3.set_title('Najczęstsze słowa kluczowe')
            ax3.set_xlabel('Liczba wystąpień')
        
        # 4. Pokrycie sekcji
        coverage_data = {
            'Z identyfikatorem': self.analysis_results["coverage"]["sections_covered"],
            'Nieznane': self.analysis_results["coverage"]["unknown_sections"]
        }
        ax4.bar(coverage_data.keys(), coverage_data.values(), color=['green', 'orange'])
        ax4.set_title('Pokrycie sekcji')
        ax4.set_ylabel('Liczba chunków')
...
def compare_chunking_strategies(
    ...
    """
    Porównuje różne strategie chunkingu.
    ...
    for name, chunker in strategies:
        try:
            ...
            analyzer = ChunkingAnalyzer(docs)
